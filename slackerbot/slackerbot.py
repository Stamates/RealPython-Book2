...
# instantiate Slacker & Slack clients
slack = Slacker(GP_HOA_STARTERBOT_TOKEN)
...
def handle_command(atbot, command, channel, ts, score):
    """
    ...
    if response != "":
        if score < -0.91: # Remove offensive message
            url = "https://slack.com/api/chat.delete?token=" + GP_HOA_TEST_TOKEN + "&ts=" + ts + \
                    "&channel=" + channel
            requests.post(url)
            response = "You're inappropriate message has been removed"
        slack.chat.post_message(channel=channel, text=response, as_user=True)
            ...
            if output and 'text' in output and output['user'] != BOT_ID:
                ...
                if AT_BOT in output['text']:
                    ...
                    return True, output['text'].split(AT_BOT)[1].strip().lower(), \
                           output['channel'], output['ts'], score
                else:
                    return False, output['text'], output['channel'], output['ts'], score
...
if __name__ == "__main__":
    ...
    if slack_client.rtm_connect():
        ...
        while True:
            atbot, command, channel, ts, score = parse_slack_output(slack_client.rtm_read())
            if command and channel:
                handle_command(atbot, command, channel, ts, score)
